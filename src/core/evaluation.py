...
class MultiTaskEvaluator:
    ...
    @torch.no_grad()
    def evaluate_paired_batch(self, model, batch, return_features=False):
        """
        ...
        # --- 3. Compute Domain Gap (Cosine Similarity) ---
    ...
    def _update_domain_metrics(self, preds, true_imgs, true_climate, true_lat, true_lon, true_coords_enc, meters, B):
        ...
        if self.use_lpips:
            rgb_pred = torch.clamp(preds['reconstruction'][:, 1:4, :, :] / 3.0, -1.0, 1.0)
            rgb_true = torch.clamp(true_imgs[:, 1:4, :, :] / 3.0, -1.0, 1.0)
