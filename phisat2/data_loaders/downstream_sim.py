...
from phisat2.data_loaders.transforms import apply_spatial_transforms, normalize_tensor, upscale_to_phisat2
...
class DownstreamSimulatedDataset(Dataset):
    def __init__(
        self, 
        ...
        self.sim_mean, self.sim_std = get_norm_tensors("phisat2_sim", PHISAT2_REAL_BANDS)
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor | str]:
        ...
        sim_tensor = self._read_tif(img_path, is_mask=False)
        ...
        transformed = apply_spatial_transforms(
            [sim_tensor, mask_tensor],
            is_train=self.is_train,
            crop_size=self.crop_size
        )
        
        return {
            "phisat2_simulated": transformed[0],
            "mask": transformed[1],
            "image_id": img_path.name
        }

class DownstreamSimulatedDataModule(L.LightningDataModule):
    def __init__(
        self,
        root_dir: str | Path,
        spec: TaskSpec,             
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        print(f"[INFO] DownstreamSimulatedDataModule: Using subset_csv={self.subset_csv}")
        def get_ds(split):
            current_subset = self.subset_csv if split == "train" else None
            return DownstreamSimulatedDataset(
                self.dataset_dir,
                split=split, 
